...
def test_openai_compatible_runner_offline_fallback():
    # Test that runner handles unreachable server gracefully
    runner = OpenAICompatibleLLMRunner(name="offline_model", api_base="http://localhost:9999/v1", mock_mode=False)
    res = runner._call_llm_api([{"role": "user", "content": "hello"}])
