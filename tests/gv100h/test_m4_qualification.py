...
def test_qualification_policy_loading():
    ...
    assert policy["gates"]["spec_qa.max_fabricated_citations"]["threshold"] == 0
    assert "coding_agent.max_false_success_rate" in policy["gates"]
    assert "hardware_feasibility.max_corruption_count" in policy["gates"]
