...
- PR #13 through PR #21 were merged sequentially; this acceptance-contract slice is based on main at `aadc1cc913b1aed99a6da3181c9d8ba3ace1b3d2`.
...
- The final POC-1 acceptance-set loader is now implemented at `gv100h/spec_qa/contracts/poc1_acceptance_contract.py`; it is a fail-closed shape validator only. No independently reviewed 50-100 question acceptance manifest has been authored yet.
...
3. **Spec QA acceptance evaluation**: author and independently review the 50-100 question manifest accepted by the new contract, add raw-document retrieval/citation coverage, and run it with `USB_SPEC_QA_RAW_ROOT`; the current smoke result, acceptance loader, and source binding receipt are not complete qualification results.
